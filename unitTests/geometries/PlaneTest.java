...
class PlaneTest {
    ...
    public void testFindIntersectionsEP()
    {
        ...
        assertEquals(null, plane.findIntersectionpoints(new Ray(new Point(0, 0, 7), new Vector(3, 0, 0))),": Ray's line is outside the plane");
        ...
        assertEquals(null, plane.findIntersectionpoints(new Ray(new Point(0, 0, 0), new Vector(3, 7, 0))),"The ray included in the plane");

        // TC04: The ray not included in the plane (0 points)
        assertEquals(null, plane.findIntersectionpoints(new Ray(new Point(0, 0, 7), new Vector(3, 0, 0))),"The ray not included in the plane");
        ...
        assertEquals(1, l.size(),"Wrong number of points");


        // TC06: The ray in the plane (0 points)
        assertEquals( null, plane.findIntersectionpoints(new Ray(new Point(9, 8, 0), new Vector(7, 5, 0))),"The ray not included in the plane");

        // TC07: The ray after the plane (0 points)
        assertEquals(null, plane.findIntersectionpoints(new Ray(new Point(0,0,-3), new Vector(0, 0, -10))),"The ray not included in the plane");

        // ** Group: Ray is neither orthogonal nor parallel to the plane
        // TC08: Ray begins at the plane(0 points)
        assertEquals(null, plane.findIntersectionpoints(new Ray(new Point(1, 4, 5), new Vector(100, 100, 100))),"Ray begins at the plane");

        // TC09: Ray begins in the same point which appears as reference point in the plane(0 points)
        assertEquals(null, plane.findIntersectionpoints(new Ray(new Point(1, 0, 0), new Vector(-6, 2, 0))),"Ray begins in the same point which appears as reference point in the plane");
    }
}